...
local function registerInstanceTests(Runner)
    ...
    Runner.Register("ClickDetector Simulation (fireclickdetector)", function()
        ...
        detector:Destroy()
        if not clicked then return false, "fireclickdetector failed to trigger MouseClick" end
        return true
    end)
    ...
    Runner.Register("Full Instance Hierarchy Retrieval (getinstances)", function()
        ...
        if type(instList) ~= "table" or #instList == 0 then
            return false, "getinstances failed to return populated instance table"
        end
        return true
    end)
end
